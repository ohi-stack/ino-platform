# Changelog

## 1.3.0-dev

- Introduced the canonical modular repository structure under `/plugin`.
- Added module directories for membership, identity, genealogy, BuddyPress, treasury, grants, housing, governance, certificates, and volunteers.
- Added dedicated directories for admin code, shared includes, assets, templates, documentation, screenshots, and tests.
- Added modular bootstrap and loader scaffolding.
- Preserved the existing v1.2.0 root implementation during migration.

## 1.2.0

- Added modern member profiles and cover images.
- Added BuddyPress compatibility and fallback social connections.
- Added family relationships and family-tree display.
- Added Identity, Ancestry & Peoplehood Registry.
- Added upgrade-safe database migrations and automatic page generation.
