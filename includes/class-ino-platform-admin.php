...
class INO_Platform_Admin {
    ...
    public static function register_menu() {
        ...
        $items = array(
            ...
            'ino-platform-programs'=>'Programs',
            'ino-platform-volunteers'=>'Volunteers',
            'ino-platform-certificates'=>'Certificates',
            'ino-platform-notifications'=>'Notifications',
            ...
            'ino-platform-production'=>'Production Checklist',
            'ino-platform-documentation'=>'Documentation',
            'ino-platform-status'=>'System Status',
    ...
    public static function dashboard() {
        ...
        $counts = array(
            ...
            'Program Applications'=>(int)$wpdb->get_var("SELECT COUNT(*) FROM {$p}ino_program_applications"),
            'Volunteer Entries'=>(int)$wpdb->get_var("SELECT COUNT(*) FROM {$p}ino_volunteer_hours"),
            'Certificates'=>(int)$wpdb->get_var("SELECT COUNT(*) FROM {$p}ino_certificates"),
            'Unread Notifications'=>(int)$wpdb->get_var("SELECT COUNT(*) FROM {$p}ino_notifications WHERE is_read=0"),
        ...
        echo '<div class="ino-admin"><div class="ino-hero"><div class="ino-kicker">Indigenous Nation of Onegodia</div><h1>INO Platform Command Dashboard</h1><p>Membership, identity, genealogy, programs, volunteering, certificates, community connections, grants, housing, documents, and governance in one control panel.</p></div><div class="ino-grid">';
    ...
    public static function module($label,$slug) {
        echo '<div class="ino-admin"><div class="ino-hero"><div class="ino-kicker">INO Platform Module</div><h1>'.esc_html($label).'</h1><p>Manage '.esc_html(strtolower($label)).' records, workflows, permissions, and reports.</p></div><div class="ino-panel"><h2>Module Status</h2><table class="ino-table"><thead><tr><th>Capability</th><th>Status</th></tr></thead><tbody><tr><td>'.esc_html($label).' module registered</td><td><span class="ino-badge">Active</span></td></tr></tbody></table>';
        ...
        if ($slug === 'ino-platform-buddypress') { echo '<div class="ino-note">When BuddyPress is active, INO uses its friendship, avatar, cover-image, activity, group, and extended-profile systems. Without BuddyPress, INO uses its own connection and family relationship tables.</div>'; }
        if ($slug === 'ino-platform-production') { echo '<div class="ino-note">A feature may be marked operational only when it is implemented, documented, tested, permission-controlled, and repeatable.</div>'; }
        if ($slug === 'ino-platform-status') { echo '<div class="ino-note">Current database schema version: '.esc_html(INO_PLATFORM_VERSION).'. Automatic page installation and schema upgrades run when the plugin version changes.</div>'; }
        echo '</div></div>';
    }
}
